...
void AnalyzerAllSteps::beginJob() {
     ...
     TFileDirectory dir_RECO_AntiLambda = dir_RECO.mkdir("RECO_AntiLambda");
     histos_th1f["h_RECO_AntiLambda_pt"] = dir_RECO_AntiLambda.make<TH1F>(b+"h_RECO_AntiLambda_pt", "; #bar{#Lambda} pT (GeV); #entries ",200,0,20);
     histos_th1f["h_RECO_AntiLambda_eta"] = dir_RECO_AntiLambda.make<TH1F>(b+"h_RECO_AntiLambda_eta", "; #bar{#Lambda} #eta; #entries ",200,-10,10);
     histos_th1f["h_RECO_AntiLambda_phi"] = dir_RECO_AntiLambda.make<TH1F>(b+"h_RECO_AntiLambda_phi", "; #bar{#Lambda} #phi (rad); #entries ",200,-4,4);
     histos_th2f["h2_RECO_AntiLambda_vx_vy"] = dir_RECO_AntiLambda.make<TH2F>(b+"h2_RECO_AntiLambda_vx_vy", "; #bar{#Lambda} vx (cm); #bar{#Lambda} vy (cm)",400,-200,200,400,-200,200);
     histos_th2f["h2_RECO_AntiLambda_vx_vz"] = dir_RECO_AntiLambda.make<TH2F>(b+"h2_RECO_AntiLambda_vx_vz", "; #bar{#Lambda} vx (cm); #bar{#Lambda} vz (cm)",400,-200,200,800,-400,400);
     histos_th1f["h_RECO_AntiLambda_lxy"] = dir_RECO_AntiLambda.make<TH1F>(b+"h_RECO_AntiLambda_lxy", "; #bar{#Lambda} lxy(beamspot, #bar{#Lambda} vertex) (cm); #entries ",200,0,200);
     histos_th1f["h_RECO_AntiLambda_vz"] = dir_RECO_AntiLambda.make<TH1F>(b+"h_RECO_AntiLambda_vz", "; #bar{#Lambda} vz(Ks vertex) (cm); #entries ",600,-300,300);
     histos_th1f["h_RECO_AntiLambda_dxy"] = dir_RECO_AntiLambda.make<TH1F>(b+"h_RECO_AntiLambda_dxy", "; #bar{#Lambda} dxy(beamspot) (cm); #entries ",400,-20,20);

     TFileDirectory dir_RECO_AntiS = dir_RECO.mkdir("RECO_AntiS");
     histos_th1f["h_RECO_AntiS_pt"] = dir_RECO_AntiS.make<TH1F>(b+"h_RECO_AntiS_pt", "; #bar{S} pT (GeV); #entries ",200,0,20);
     histos_th1f["h_RECO_AntiS_eta"] = dir_RECO_AntiS.make<TH1F>(b+"h_RECO_AntiS_eta", "; #bar{S} #eta; #entries ",200,-10,10);
     histos_th1f["h_RECO_AntiS_phi"] = dir_RECO_AntiS.make<TH1F>(b+"h_RECO_AntiS_phi", "; #bar{S} #phi (rad); #entries ",200,-4,4);
     histos_th2f["h2_RECO_AntiS_vx_vy"] = dir_RECO_AntiS.make<TH2F>(b+"h2_RECO_AntiS_vx_vy", "; #bar{S} vx (cm); #bar{S} vy (cm)",400,-200,200,400,-200,200);
     histos_th2f["h2_RECO_AntiS_vx_vz"] = dir_RECO_AntiS.make<TH2F>(b+"h2_RECO_AntiS_vx_vz", "; #bar{S} vx (cm); #bar{S} vz (cm)",400,-200,200,800,-400,400);
     histos_th1f["h_RECO_AntiS_vz"] = dir_RECO_AntiS.make<TH1F>(b+"h_RECO_AntiS_vz", "; #bar{S} vz(Ks vertex) (cm); #entries ",600,-300,300);
     histos_th1f["h_RECO_AntiS_dxy"] = dir_RECO_AntiS.make<TH1F>(b+"h_RECO_AntiS_dxy", "; #bar{S} dxy(beamspot) (cm); #entries ",400,-20,20);
...
cout << "0" << endl;
  ...
  if(h_V0L.isValid()){
	...
	FillHistosRECOAntiLambda(L,beamspot);
      }
  }
...
void AnalyzerAllSteps::FillHistoAllGranddaughtersCorrect(const reco::Candidate  * genParticle, edm::Handle<vector<reco::VertexCompositeCandidate> > h_V0Ks, edm::Handle<vector<reco::VertexCompositeCandidate> > h_V0L, int eventId){
	...
	if(KsRECOMatched && AntiLRECOMatched){//found for this single antiS both a reconstructed Ks and antiL daughter
		cout << "found for this single GEN antiS both a reconstructed Ks and antiL daughter (based on deltR matching); event info: id " << eventId << endl;
...
void AnalyzerAllSteps::FillHistosRECOKs(const reco::VertexCompositeCandidate * RECOKs, TVector3 beamspot){
...
void AnalyzerAllSteps::FillHistosRECOAntiLambda(const reco::VertexCompositeCandidate * RECOAntiLambda, TVector3 beamspot){
	TVector3 AntiLambdaCreationVertex(RECOAntiLambda->vx(),RECOAntiLambda->vy(),RECOAntiLambda->vz());
	double Lxy = lxy(beamspot,AntiLambdaCreationVertex);
	TVector3 AntiLambdaMomentum(RECOAntiLambda->px(),RECOAntiLambda->py(),RECOAntiLambda->pz());
	double dxy = dxy_signed_line_point(AntiLambdaCreationVertex,AntiLambdaMomentum,beamspot);
	histos_th1f["h_RECO_AntiLambda_pt"]->Fill(RECOAntiLambda->pt());	
	histos_th1f["h_RECO_AntiLambda_eta"]->Fill(RECOAntiLambda->eta());	
	histos_th1f["h_RECO_AntiLambda_phi"]->Fill(RECOAntiLambda->phi());	
	histos_th2f["h2_RECO_AntiLambda_vx_vy"]->Fill(RECOAntiLambda->vx(),RECOAntiLambda->vy());
	histos_th2f["h2_RECO_AntiLambda_vx_vz"]->Fill(RECOAntiLambda->vx(),RECOAntiLambda->vz());
	histos_th1f["h_RECO_AntiLambda_lxy"]->Fill(Lxy);	
	histos_th1f["h_RECO_AntiLambda_vz"]->Fill(RECOAntiLambda->vz());	
	histos_th1f["h_RECO_AntiLambda_dxy"]->Fill(dxy);	
}
...
void AnalyzerAllSteps::FillHistosRECOAntiS(const reco::VertexCompositeCandidate * RECOAntiS, TVector3 beamspot, TVector3 beamspotVariance, edm::Handle<vector<reco::GenParticle>> h_genParticles, int eventId){
	...
	histos_th1f["h_RECO_AntiS_error_lxy"]->Fill(deltaRDaughters);
	...
	histos_th1f["h_RECO_AntiS_m"]->Fill(RECOAntiS->mass());
	if(Lxy > 1.9)histos_th1f["h_RECO_AntiS_m_lxy_cut"]->Fill(RECOAntiS->mass());
	
	if(deltaRDaughters < 3)histos_th1f["h_RECO_AntiS_m_deltaR_cut"]->Fill(RECOAntiS->mass());	
	if(deltaRDaughters < 3 && Lxy > 1.9)histos_th1f["h_RECO_AntiS_m_lxy_and_deltaR_cut"]->Fill(RECOAntiS->mass());	
	if(deltaRDaughters < 3 && Lxy > 1.9)histos_th2f["h2_RECO_AntiS_m_lxy_and_deltaR_cut_iEvent"]->Fill(RECOAntiS->mass(), eventId);	
	if(deltaRDaughters < 3 && Lxy > 1.9)cout << "found a RECO antiS in this event which surives the deltaR cut and the Lxy cut; event info: id " << eventId << endl;	
	if(deltaRDaughters < 3 && Lxy > 1.9 && error_Lxy < 0.1)histos_th1f["h_RECO_AntiS_m_lxy_and_error_lxy_and_deltaR_cut"]->Fill(RECOAntiS->mass());	
	...
	double bestDeltaRLxy_and_deltaRDaughtersCut = 999;
		...
		histos_th2f["h2_RECO_AntiS_m_lxy_and_deltaR_cut_vs_deltaR_best_GEN_match"]->Fill(RECOAntiS->mass(),bestDeltaRLxy_and_deltaRDaughtersCut);
	}
	...
	double bestDeltaRLxy_and_errorLx_and_deltaRDaughtersCut = 999;
        if(deltaRDaughters < 3 && Lxy > 1.9 && error_Lxy < 0.1){
		if(h_genParticles.isValid()){
			for(unsigned int i = 0; i < h_genParticles->size(); ++i){//loop all genparticlesPlusGEANT
				const reco::Candidate * genParticle = &h_genParticles->at(i);
				if(genParticle->pdgId() == pdgIdAntiS){	
		...
		//now fill this bestDeltaR in a 2D histo with the mass to see if the ones with properly RECO mass also have a small deltaR...
		histos_th2f["h2_RECO_AntiS_m_lxy_and_error_lxy_and_deltaR_cut_vs_deltaR_best_GEN_match"]->Fill(RECOAntiS->mass(),bestDeltaRLxy_and_errorLx_and_deltaRDaughtersCut);
	}

		
	if(abs(deltaPhiDaughters) < 2.5 && abs(deltaPhiDaughters) > 1)histos_th1f["h_RECO_AntiS_m_deltaPhi_cut"]->Fill(RECOAntiS->mass());	
	if(abs(deltaPhiDaughters) < 2.5 && abs(deltaPhiDaughters) > 1 && Lxy > 1.9)histos_th1f["h_RECO_AntiS_m_lxy_and_deltaPhi_cut"]->Fill(RECOAntiS->mass());	
	if(abs(deltaPhiDaughters) < 2.5 && abs(deltaPhiDaughters) > 1 && Lxy > 1.9 && error_Lxy < 0.1)histos_th1f["h_RECO_AntiS_m_lxy_and_error_lxy_and_deltaPhi_cut"]->Fill(RECOAntiS->mass());	
