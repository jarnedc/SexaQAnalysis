...
#MinBias as data all filters ran
rangeOfStrings=[str(i) for i in range(1,200)] #normal range is 1 to 148 but it gets killed all the time
tupleOfFiles=tuple(['file:///pnfs/iihe/cms/store/user/jdeclerc/MinBias_TuneCUETP8M1_13TeV-pythia8/MinBias_asData_150718/180715_164309/0000/V0_correlation_SingleMuon_single_file_allFilters_' + x + '.root' for x in rangeOfStrings])
...
#rangeOfStrings=[str(i) for i in range(1,70)] #normal range is 1 to 148 but it gets killed all the time
#tupleOfFiles=tuple(['file:///pnfs/iihe/cms/store/user/jdeclerc/SingleMuon/SingleMuon_Run2016G/180629_143246/0000/MC_events_skimmed_' + x + '.root' for x in rangeOfStrings])


#MinBias only first filter ran
#rangeOfStrings=[str(i) for i in range(1,199)] #normal range is 1 to 148 but it gets killed all the time
#tupleOfFiles=tuple(['file:///pnfs/iihe/cms/store/user/jdeclerc/MinBias_TuneCUETP8M1_13TeV-pythia8/MinBias_asData_150718_only_firstFilter/180715_164611/0000/V0_correlation_ZeroBias_single_file_' + x + '.root' for x in rangeOfStrings])
...
process.TFileService = cms.Service('TFileService',
    fileName = cms.string('analysed_MinBias_asData_LambdaKshort_correlation_allFilters.root')
)
